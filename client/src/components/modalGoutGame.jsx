            ...
            const fetchItemsFlavors = async () => {
                try {
                    const response = await axios.get(
                        "http://localhost:5555/baby-food-game/get-all-flavors"
                    );
    ...
    return (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
            <div className="bg-top-gray-brown-light rounded-lg shadow-lg max-h-screen overflow-y-auto w-[80vw] h-[80vh] relative">
                <button className="absolute top-2 right-2 text-gray-700 hover:text-gray-900" onClick={onClose}>&times;</button>
                <h2 className="text-xl font-bold p-4">Après l'odorat le goût</h2>
                <form className="p-4">
                    {['pot1', 'pot2', 'pot3'].map(pot => (
                        <div key={pot} className="mb-4">
                            <h3 className="font-bold">{pot.toUpperCase()}</h3>
                            {itemsFlavors.map(flavor => (
                                <div key={flavor} className="flex items-center">
                                    <input
                                        ...
                                        disabled={!selectedFlavors[pot].includes(flavor) && selectedFlavors[pot].length >= 4}
                                    />
                                    <label htmlFor={`${pot}-${flavor}`} className="ml-2">{flavor}</label>
                                </div>
                            ))}
                        </div>
                    ))}
